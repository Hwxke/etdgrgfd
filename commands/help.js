  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (info) {
        ...
        const embed = new EmbedBuilder()
          .setColor(client.config.embedColor)
          .setTitle("MusicX Dashboard")
          .setThumbnail(client.user.displayAvatarURL())
          .setDescription("I Am MusicX. A Discord Music Bot Made To Provide You With Many Breathtaking Features And Quality Music")
          .addFields([
            { name: `${lang.msg33}`, value: commands.map(x => `\`/${x.name}\``).join(' | ') }
          ])
          .setTimestamp()
          .setFooter({ text: `MusicX ❤️` })
